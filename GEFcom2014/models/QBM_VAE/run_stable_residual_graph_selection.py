...
"""Run the VS-only stable residual-MI graph development experiment."""
...
EXPERIMENTS = (
    (
        "wind",
        ROOT_DIR / "configs" / "paper" / "fa_bm_vae_wind_lanchor_stable_residual_mi.json",
    ),
    (
        "opsd-wind",
        ROOT_DIR / "configs" / "paper" / "fa_bm_vae_opsd_wind_anchor_stable_residual_mi.json",
    ),
)


def scenario_path(track, seed):
    stem = "scenarios_%s_QBMVAE_2_dev_sresmi_sa_%d_100_VS.pickle" % (
        track,
        seed,
    )
...
def main():
    args = parse_args()
    log_dir = ROOT_DIR / "export" / "experiment_logs" / "stable_residual_graph_selection"
    ...
    print("VS-only stable residual graph selection complete", flush=True)
