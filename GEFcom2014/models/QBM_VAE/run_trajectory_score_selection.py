...
"""Run the VS-only trajectory-score regularized FA-BM-VAE experiment."""
...
LABEL = "dev_tsr"
EXPERIMENTS = (
    ("wind", ROOT_DIR / "configs" / "paper" / "fa_bm_vae_wind_lanchor_trajectory_score.json"),
    ("opsd-wind", ROOT_DIR / "configs" / "paper" / "fa_bm_vae_opsd_wind_anchor_trajectory_score.json"),
)
...
def main():
    args = parse_args()
    log_dir = ROOT_DIR / "export" / "experiment_logs" / "trajectory_score_selection"
    ...
    run_logged(
        [sys.executable, "-u", "-m", "GEFcom2014.forecast_quality.select_stable_residual_graph", "--candidate-label", LABEL, "--seeds", *[str(seed) for seed in args.seeds], "--repetitions", str(args.repetitions), "--output-dir", str(ROOT_DIR / "export" / "trajectory_score_selection")],
        log_dir / "selection.out.log",
        log_dir / "selection.err.log",
    )
    print("VS-only trajectory-score selection complete", flush=True)
